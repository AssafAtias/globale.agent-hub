...
- **Status vocabulary is shared.** One `StatusPill` and one status-dot style are used by Agents cards, the dashboard, and Runners, all driven by `stateStyles`/status colors, so status looks consistent across screens. **`StatusPill` renders `stateStyles[state].label` verbatim** — the canonical labels are therefore **Working / Reviewing / Queued / Idle / Blocked** (from [palette.ts:17](../../../apps/client/src/components/dashboard/palette.ts#L17)). Where this doc says "Running"/"Failed" in prose, those are the conceptual run states that map to `working`/`blocked` via the existing `stateFor`; the pill shows the `stateStyles` label, not literal "Running"/"Failed". Do **not** invent a parallel label set.
...
2. **Meta line (new):** a single muted line `「{model} · {trigger}」`, e.g. "Sonnet 4.6 · on Jira label", where trigger is summarized from `triggerRules`. **`triggerRules` is stored as a JSON string** on the `Agent` model (shape `{events: string[], branchFilter?, jiraLabel?}`) — it must be JSON-parsed first, reusing the card's existing local `parse<T>()` helper (same pattern as `repos`/`skills`). The summary prefers `jiraLabel` ("on Jira label") else the first event. Replaces putting `type` and `model` as chips.
...
5. **Sparkline (new):** a thin row of bars colored by `runMarker[status]` from `recentRunMarkers(runs, agentId, n)` with a fixed **`n = 10`**; empty/neutral when no runs. Implemented as a tiny `Sparkline` component (plain `<Box>` bars, no charting lib). The component and its test both use `n = 10`.
